                ...
                "operations": {
                  ...
                  "items": {
                    ...
                    "properties": {
                      "op": {
                        "type": "string",
                        "description": "Тип операции: set_form_props/add_group/add_field/add_command/add_button/set_item/remove_item/move_item/rename_command"
                      }
                    },
                    "required": ["op"],
                    "additionalProperties": true
                  },
                  "description": "Список операций: set_form_props/add_group/add_field/add_command/add_button/set_item/remove_item/move_item. ALWAYS prefer these over direct .form XML editing — BM API applies designer defaults (e.g. showInHeader=true) raw XML skips, and keeps ids/cross-refs consistent. add_command: name+action (handler); optional picture (CommonPicture.<Name>). add_button: name+command_name+parent_item_id; optional picture (CommonPicture.<Name>) + representation (Auto/Text/Picture/PictureAndText). rename_command: rename a form command (command_name OR command_id from get_form_rendering; new_name) — atomically updates the formCommand name and every referencing button's commandName (object reference, kept consistent); optional new_action rebinds the BSL handler procedure, optional new_title updates the display title. This is the ONLY way to rename a command: commandName on buttons is a reference property (not settable via set_item) and formCommands are not in the set_item/remove_item item tree. EVENT HANDLERS bind via set_item set:{handlers:[{event:'OnChange', handler:'MyHandler'}, ...]} on the item or form (merge-safe — leaves untouched events alone; register write-events like BeforeWrite resolve to the form's extInfo automatically) — do NOT hand-edit <handlers> in .form. set_item also reaches Buttons inside a table/form autoCommandBar (by item_id or item_name), so picture/representation on existing command-bar buttons go through set_item. The 'set' map accepts ANY scalar property of the element: showInHeader (bool), headerHorizontalAlign (HorizontalAlign enum), showInFooter, footerHorizontalAlign, titleLocation (FormItemTitleLocation enum), width, minWidth, maxWidth, enabled, readOnly, userVisible (bool or per-role map), picture, representation. Column header/footer props must be set via set_item, not by hand-editing .form."
                },
                "validation_token": {
                  "type": "string",

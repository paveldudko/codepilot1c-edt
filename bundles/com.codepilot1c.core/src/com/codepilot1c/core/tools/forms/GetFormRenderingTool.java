...
import com.codepilot1c.core.edt.geometry.FormElementNode;
import com.codepilot1c.core.edt.geometry.FormGeometryEngine.BoundingBox;
import com.codepilot1c.core.edt.geometry.FormRenderingService;
import com.codepilot1c.core.edt.geometry.FormRenderingService.FormRenderingResult;
...
/**
 * Returns a schematic PNG render of a managed form along with the bbox
 * list from {@link com.codepilot1c.core.edt.geometry.FormGeometryService}.
 *
 * <p>Second half of Gap 5 from the AM-side feedback at
 * {@code 2026-05-19-bsl-edit-and-context-gaps.md}: adds a headless PNG
 * over the Phase-3 geometry so the agent can also visually inspect the
 * form without starting a TestClient. PNG is opt-out via
 * {@code return_png=false} to keep purely-spatial checks cheap.</p>
 *
 * <p>Diff mode ({@code baseline:"HEAD"}) is deferred — the agent can
 * already approximate it by making two calls on different branches and
 * comparing the {@code items[]} bbox-by-bbox.</p>
 */
@ToolMeta(name = "get_form_rendering", category = "forms", tags = {"read-only", "workspace", "edt"})
public class GetFormRenderingTool extends AbstractTool {
    ...
    private final FormRenderingService service;

    public GetFormRenderingTool() {
        this(new FormRenderingService());
    }

    public GetFormRenderingTool(FormRenderingService service) {
    ...
    private void walkAndEmit(FormElementNode node,
                             Map<String, BoundingBox> bboxes,
                             Set<String> highlightSet,
                             JsonArray out) {
        if (node == null) return;
        BoundingBox bbox = bboxes.get(node.name());
        if (bbox != null && node.kind() != FormElementNode.Kind.FORM) {

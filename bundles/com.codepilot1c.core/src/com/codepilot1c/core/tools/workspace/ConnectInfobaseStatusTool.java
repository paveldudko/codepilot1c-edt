...
/**
 * Polls the status of a background {@code connect_infobase} job started via {@code async=true}.
 */
@ToolMeta(
        name = "connect_infobase_status",
        category = "workspace",
        mutating = false,
        tags = {"workspace", "edt"})
public class ConnectInfobaseStatusTool extends AbstractTool {

    private static final VibeLogger.CategoryLogger LOG =
            VibeLogger.forClass(ConnectInfobaseStatusTool.class);
    ...
    private static final String SCHEMA = """
            {
              ...
              "properties": {
                "job_id": {
                  "type": "string",
                  "description": "Job id returned by connect_infobase when async=true"
                }
              },
              "required": ["job_id"]
            }
    ...
    public ConnectInfobaseStatusTool() {
        this(BackgroundJobRegistry.getInstance());
    }

    ConnectInfobaseStatusTool(BackgroundJobRegistry registry) {
    ...
    protected CompletableFuture<ToolResult> doExecute(ToolParameters params) {
        ...
        switch (lookup.getKind()) {
            ...
            case EXPIRED: {
                LOG.warn("connect_infobase_status: expired job id %s", jobId); //$NON-NLS-1$
                ...
                payload.addProperty("message", "Job result retention window has elapsed"); //$NON-NLS-1$
                return CompletableFuture.completedFuture(
                        ToolResult.failure(pretty(payload)));
            }
            case UNKNOWN:
            default: {
                LOG.warn("connect_infobase_status: unknown job id %s", jobId); //$NON-NLS-1$
                return CompletableFuture.completedFuture(

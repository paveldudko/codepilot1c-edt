...
public final class QwenToolSurfaceRewriteContributor implements ToolSurfaceContributor {
    ...
    private String overrideDescription(String toolName) {
        return switch (toolName) {
            ...
            case "add_metadata_child" -> "Создаёт дочерний объект (реквизит, табличную часть, команду, макет, значение перечисления). Тип: {types:[xs:string]} или {types:[CatalogRef.Склады]}. НЕ используй v8:STRING. Для макетов: child_kind=Template, template_type=spreadsheet (по умолч.) — .mxl файл создаётся автоматически. Для значений Enum: parent_fqn=Enum.<Name>, child_kind=EnumValue."; //$NON-NLS-1$

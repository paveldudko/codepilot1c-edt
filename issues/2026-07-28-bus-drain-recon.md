...
признаёт, что процесс с нечитаемой командной строкой в принципе не атрибутируется к ИБ. **Почему скан вернул
пусто на стенде :8778 — остаётся неизвестным**; следующий отчёт будет разрешим по payload'у.
...
Живая валидация всего пакета — **pending**: пути апдейта требуют настоящей ИБ, метаданной песочницы мало.

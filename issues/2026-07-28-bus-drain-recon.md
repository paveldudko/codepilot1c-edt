...
### РЕЗУЛЬТАТ живой валидации (2026-07-30, стенд отработан и возвращён)

Стенд был передан (`6cdcd89d`), пробы выполнены на билде `-2134`, стенд возвращён (`b931fc64`).

**Подтверждено живьём:**

* **`schema_applied` присутствует на КАЖДОМ исходе** — проверено на четырёх разных путях: async-приём
  (`scheduled`, false), error внутри джобы (`UPDATE_BLOCKED_BY_WEBSERVER`, false), синхронный отказ (false),
  успешный эксклюзивный апдейт (**true**). Это ровно та половина контракта, которая устраняла двусмысленность
  «поля нет / билд старый» — закрыта.
* **`schema_applied:true` + `status:"updated"`** на эксклюзивном применении — закрыто.
* Блокирующий `update_infobase_status(wait_for_completion=true, timeout_seconds=1500)` отработал и вернул
  терминальный вердикт с вложенным result'ом.

**НЕ проверено, и на этом стенде не проверяемо в принципе — записано честно, а не как «сделано»:**

* **§4 (ожидание дольше старого капа ~660s).** Апдейт здесь занимает 3–4 с: то 13-минутное окно было ПЕРВЫМ
  полным схемным апдейтом свежей копии, и это состояние израсходовано. По короткой джобе отличить «потолок
  поднят» от «потолок всё ещё 600 и молча зажал 1500» **невозможно**. Для проверки нужен свежий robocopy
  большой ИБ с непримененной схемой — один длинный апдейт, отдельного стенда не требуется.
* **EQUAL-skip.** `skip_if_current` пропускает только при EQUAL, а эта ИБ к EQUAL не приходит (см. ниже),
  поэтому вызов штатно ушёл в полный апдейт (`skipped:false`). Нужна сошедшаяся ИБ — секундная проверка.
* **`dynamic_only`.** Фантом в окне не пришёл ни разу, эксклюзив брался мгновенно; насильно не воспроизводил.

**Побочно закрыт открытый вопрос орка про висящий `NOT_EQUAL`** — и не в пользу подсказки нашего же тула.
Два эксклюзивных апдейта (4 с и 3 с), оба `updated:true` + `schema_applied:true`, **`dynamic_only` не выставлен
ни разу** ⇒ гипотеза «применилось динамически» исключена: флаг ставится именно тогда, когда эксклюзив не взялся.
Плюс 3–4 секунды означают, что реструктурировать было нечего, что совпадает с физической проверкой дева
(канареечные записи строковым ключом). Значит это `equality_state`, не сходящийся после НАСТОЯЩЕГО эксклюзивного
апдейта — третий известный режим (`2026-07-13-update-infobase-completes-but-equality-state-never-converges-third-mode.md`),
дефект отображения, отдельный от этого пакета. Заметку орка про false-negative переписывать не нужно.
Наш канал при этом сработал правильно: `updated:true` приехал вместе с громким `equality_state_after_warning`
«не гоняй тот же апдейт в цикле, он не сойдётся».

**Ловушка окружения, стоит помнить:** первый заход отказал с `UPDATE_BLOCKED_BY_WEBSERVER`. Гард машинно-широкий
(атрибуция по списку процессов ненадёжна намеренно), а запущенный httpd — `apache-stack-1` с `httpd-1c.conf`,
чья единственная публикация (`published\agent-current\default.vrd`) смотрит на
`File="C:\1C\Dudko\db\Branches\BF-13334"`, то есть на ДРУГУЮ ИБ. Верное лекарство назвало само сообщение —
`allow_webserver_running=true`. Apache **не останавливался**: это сломало бы живую публикацию чужой задачи на
общей машине. Разбираться чтением vrd, а не подбором флагов.

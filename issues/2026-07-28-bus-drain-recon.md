...
1. **Терпимого алиаса вложенной формы FQN подсистемы в билде НЕТ.** Живьём не резолвится ни одна точечная
   форма: `Subsystem.<Parent>.<Child>` → `METADATA_PARENT_NOT_FOUND` («must be marker/name pairs»), парная
   `Subsystem.<Parent>.Subsystem.<Child>` → `METADATA_NOT_FOUND`. То есть §B4 «вложенную принимаем как алиас»
   реализовано не было, а сообщение об ошибке отправляет вызывающего к форме, которая тоже падает.
...
- **`scan_metadata_index` молча игнорирует неизвестный параметр.** Фильтр называется `scope`; вызов с
  `kinds='Subsystem'` вернул все объекты проекта без единого предупреждения, то есть выглядел как
  отфильтрованный. Тот же класс, что закрытый footgun `grep` (`e01c1df`, честный хинт на нулевой матч):
  правильное лечение — не строгий отказ на любой лишний параметр, а честный хинт, когда неизвестный
  параметр похож на алиас существующего фильтра.
